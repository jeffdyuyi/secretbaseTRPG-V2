...
import { SessionData, INITIAL_SESSION, CloudConfig } from './types';
...
import { FileText, Download, Calendar as CalendarIcon, Dice5, Copy, Check, AlertTriangle, Rocket, Cloud, RefreshCw, Settings, Share2, Upload, Database } from 'lucide-react';
import { startOfWeek, format, subDays, addDays, parseISO } from 'date-fns';

function App() {
  const [activeTab, setActiveTab] = useState<'generator' | 'schedule' | 'satellite' | 'settings'>('generator');
  ...
  const [cloudConfig, setCloudConfig] = useState<CloudConfig>(() => {
      ...
      const defaultConf = { apiToken: '', datasheetId: '', satelliteDatasheetId: '', publicShareUrl: '', enabled: false };
  ...
  const refreshCloudData = async () => {
      ...
      try {
          ...
          // 2. Fetch Satellite Table (if configured)
          if (cloudConfig.satelliteDatasheetId) {
             try {
                satelliteData = await VikaService.fetchSessions(cloudConfig.apiToken, cloudConfig.satelliteDatasheetId);
             } catch (e) {
                console.warn("Failed to fetch satellite table", e);
             }
          }
  ...
  const handleSaveSession = async (mode: 'create' | 'update') => {
    ...
    if (cloudConfig.enabled) {
        ...
        try {
            ...
            const targetTableId = (isSatellite && cloudConfig.satelliteDatasheetId) 
                                ? cloudConfig.satelliteDatasheetId 
                                : cloudConfig.datasheetId;
            ...
            await refreshCloudData();
            alert(isSatellite ? '已同步到云端卫星站！' : '已同步到云端日程！');
            setEditingId(null);
        } catch (e: any) {
  ...
  const handleEditSession = (id: string) => {
    const target = sessions.find(s => s.id === id) || satellites.find(s => s.id === id);
    if (target) {
        setFormData(target);
        setEditingId(id);
          ...
          <div className="flex justify-between h-16">
            ...
            <div className="flex space-x-1 sm:space-x-2 items-center">
              ...
              <button onClick={() => setActiveTab('schedule')} className={`p-2 sm:px-3 sm:py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'schedule' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:text-slate-700'}`}>
                 <CalendarIcon size={18} className="sm:mr-2 inline"/> <span className="hidden sm:inline">日程</span>
              </button>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-1">日程表 ID (本周开团)</label>
                                <input 
                                    type="text" 
